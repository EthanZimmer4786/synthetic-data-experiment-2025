...
def reduce_data(x, y):
    """If X argument contains duplicate values, Y values corresponding to said X values will be averaged resulting in a shorter list"""

    x_set = sorted(list(set(x)))
    y2 = [([], x) for x in x_set]

    for i in range(len(x)):
        y2[x_set.index(x[i])][0].append(y[i])
